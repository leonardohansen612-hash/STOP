...
export function randomLetter(excluded=[]){
  const blocked=new Set((excluded||[]).map(x=>String(x).toUpperCase()));
  const available=LETTERS.filter(letter=>!blocked.has(letter));
